...
def _render_chart(part: str, target):
    """Render a <plotly_chart> block safely into the target container."""
    m = _CHART_TAG_RE.search(part)
    raw = m.group(1).strip() if m else part.strip()
    spec = _parse_chart_json(raw)

    with target.client, target:
        if spec is None:
            ...
            with ui.element('div').style(
                ...
                ui.label('Chart parse error').classes('text-[11px] font-bold text-red-800 mb-1')
                ui.label(preview).classes('text-[10px] text-red-600 font-mono break-all')
            return

        with ui.element('div').style(
            'width:100%;margin-top:8px;border-radius:12px;overflow:hidden;border:1px solid #f3f4f6;'
        ):
...
def render_ai_block(raw_text: str, container):
    def _render_content_loop(txt, target):
        ...
        for part in parts:
            ...
            if part.startswith('<plotly_chart>'):
                ...
                with target.client, target:
                    content = re.search(r'<explanation>(.*?)</explanation>', part, re.DOTALL).group(1).strip()
                    with ui.expansion('🔍 Deep Analysis', icon='query_stats').classes('w-full bg-blue-50 border border-blue-200 rounded-xl text-xs mt-2'):
                        with ui.element('div').classes('p-3') as exp_container:
                            _render_content_loop(content, exp_container)
            else:
                with target.client, target:
    ...
    with container.client, container:
        with ui.element('div').classes('flex gap-2.5 items-start w-full'):
            ...
            with ui.element('div').classes('flex flex-col gap-1 flex-1 min-w-0'):
                ...
                # Explicitly create message container inside the bubble
                msg_inner = ui.element('div').classes('w-full flex flex-col gap-2')
                with msg_inner:
                    _render_content_loop(raw_text, msg_inner)
...
if __name__ in {'__main__', '__mp_main__'}:
    ui.run(title=f'{APP_TITLE} {APP_SUBTITLE}', port=5174, show=False)